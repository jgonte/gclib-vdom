...
export default abstract class VirtualDomComponent implements LifecycleHooks {
    ...
    onNodeDidUpdateChildren?: (node: Node, updatedChildren: UpdatedChildren) => void;
    ...
    bindLifecycleCallbacks() {
        ...
        this.nodeDidUpdateChildren = this.nodeDidUpdateChildren.bind(this);
    }
    ...
    nodeDidUpdateChildren(node: Node, updatedChildren: UpdatedChildren) {

        //callMixins(this, 'nodeDidUpdateChildren', node);

        if (this.onNodeDidUpdateChildren) {

            this.onNodeDidUpdateChildren(node, updatedChildren);
        }
    }
