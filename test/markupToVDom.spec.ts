...
describe("parse a string and covert to a virtual dom tree tests", () => {
    ...
    `<gcl-list-item value=1>       
        ...
        const vnode = markupToVDom(markup, 'html', { excludeTextWithWhiteSpacesOnly: true }) as VirtualNode;
    ...
    `;
        ...
        expect((vnode.children[0] as VirtualNode).name).toEqual('span');

        expect((vnode.children[1] as VirtualNode).name).toEqual('gcl-text');

        expect((vnode.children[2] as VirtualNode).name).toEqual('gcl-text');

        expect((vnode.children[3] as VirtualNode).name).toEqual('gcl-text');

        expect((vnode.children[4] as VirtualNode).name).toEqual('img');
    });
    ...
    //     const vnode = markupToVDom(markup, 'xml', { excludeTextWithWhiteSpacesOnly: true}) as VirtualNode;
